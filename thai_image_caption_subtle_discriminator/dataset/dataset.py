...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if "correct_image" in sample:
            correct_image = sample["correct_image"]
            correct_image =  torch.from_numpy(correct_image.transpose((2, 0, 1)).astype(np.float32)) if len(correct_image.shape) == 3 else torch.from_numpy(np.stack((correct_image, correct_image, correct_image), axis=0).astype(np.float32))
            sample["correct_image"] = correct_image
